...
<#
    ...
    .EXAMPLE
        ...
        Parameter – Übersicht
            ...
            Quell-Exchange-Version (z. B. 15.1). Default: 15.1.
            -TargetVersion <string>
            Ziel-Exchange-Version (z. B. 15.2). Default: 15.2.
            -IncludeDbs <string[]>
            Optional: Liste von Quell-DBs, auf die gefiltert wird.

            Pfadsteuerung:
            -PathMode <MirrorSource|BasePerDb>
            Pfadermittlung. Default: MirrorSource.
            -DbBase <string>
            Basisordner für DBs (Default: E:\Datenbanken).
            -LogBase <string>
            Basisordner für LOGs (Default: E:\Logs).
            -DriveMap <hashtable>
            Remapping von Laufwerken, z. B. @{ 'D:'='E:' }.

            Aktionen (nur mit -Approve schreibend):
            -PrepareFolders
            Legt Zielordner (DB/LOG) auf dem Zielserver an.
            -CreateDatabases
            Erstellt Mailbox-Datenbanken inkl. Pfade (optional Mount/Exclude).
            -CompareSettings
            Vergleicht DB-Settings Quelle ↔ Ziel (Scope siehe -SettingsScope).
            -ApplySettings
            Wendet gefundene Diffs auf Ziel-DBs an (Set-MailboxDatabase).
            -QueueMoves
            Erstellt MigrationBatch für Benutzerpostfächer (lokale Moves).
            -QueueSystemMailboxBatch
            Erstellt MigrationBatch für Systemmailboxen (Arbitration + optional Discovery).
            -Interactive
            Interaktive Anpassung der Zielpfade vor Ausführung.
            -Approve
            Sicherheitshebel – ohne diesen keine schreibenden Aktionen.

            Optionen – DB-Erstellung:
            -MountAfterCreate (Switch, Default: $true)
            Mount nach Erstellung.
            -ExcludeFromProvisioning (Switch, Default: $true)
            DB vom Provisioning ausschließen.

            Optionen – Settings-Abgleich:
            -SettingsScope <string[]>
            Gruppen: Quotas, Retention, Client, Maintenance, OAB, All (Default).
            -ExportDiffs <string>
            CSV-Pfad für Settings-Diffs.

            Optionen – Moves (Benutzerpostfächer):
            -BatchNamePrefix <string>
            Präfix für Batchnamen (Default: FrontierMove).
            -BadItemLimit <int>
            Max. fehlerhafte Elemente pro Postfach.
            -NotifyEmail <string>
            E-Mail für Batch-Benachrichtigungen.
            -AutoStart (Switch)
            Startet MigrationBatch nach Erstellung.
            -KeepCsv (Switch)
            Behält die generierte CSV-Datei (Debug).
            -FallbackTargetDb <string>
            Fallback-Ziel-DB für nicht gemappte Quell-DBs.
            -ForceTargetDb <string>
            Erzwingt eine Ziel-DB für alle regulären Moves.
            -Arbitration (Switch)
            Erstellt Batch für Arbitration-Mailboxen separat.
            -SplitArbitrationBySourceDb (Switch)
            (Optionales Verhalten für Arbitration-Batches).

            Optionen – Systemmailbox-Batch:
            -SystemMailboxTargetDb <string>
            Ziel-DB für Systemmailboxen.
            -IncludeDiscoverySystemMailbox (Switch)
            Nimmt Discovery Search Mailbox in den Batch auf.

            Review & Export:
            -ExportPlan <string>
            CSV-Pfad für Export des Plans.

            Debug / CAS:
            -DebugOAB (Switch)
            Detailausgabe zur OAB-Auflösung.
            -CompareCasUrls (Switch)
            Vergleicht CAS-/VD-URLs zwischen Quelle und Ziel.
            -ApplyCasUrls (Switch)
            Übernimmt CAS-/VD-URLs vom Quellserver.
            -SourceCasServer <string>
            CAS-Server für Vergleich (Quelle).
            -TargetCasServer <string>
            CAS-Server für Vergleich (Ziel).
            -CasShowAll (Switch)
            Zeigt alle Eigenschaften inkl. Gleichständen.
        ----------------------------------------------------------------------------
        Wichtiges Verhalten
        ...
        Beispiel-Workflows
         ...
         (4) Postfach-Migration per MigrationBatch:
             Arbitration / System Mailboxen mailboxes:
             (Batch anlegen, kein Start):
             .\Exchange_Migration_Script.ps1 -QueueSystemMailboxBatch -IncludeDiscoverySystemMailbox -BadItemLimit 10 -NotifyEmail admin@domain.com -Approve

             Arbitration / System Mailboxen mailboxes (optional):
             (Batch anlegen, kein Start, bestimmte ZielDB):
             ...
             Erzwinge-Ziel-DB für alle Quell-DBs:
             .\Exchange_Migration_Script.ps1 -QueueMoves -BatchNamePrefix "mailboxmove" -BadItemLimit 10 -NotifyEmail admin@domain.com -Approve -FallbackTargetDb "MDB02"
        ============================================================================
    .NOTES
...
param(
  ...
  # Systemmailbox-Batch (NEU in 1.2.3)
  [switch]$QueueSystemMailboxBatch,
  [switch]$IncludeDiscoverySystemMailbox,
  [string]$SystemMailboxTargetDb
)

# --- Helpers / Environment ----------------------------------------------------

function Test-ExchangeEnvironment {
  Write-Host "Exchange Migration Script starting..." -ForegroundColor Cyan
  if (-not (Get-Command Get-ExchangeServer -ErrorAction SilentlyContinue)) {
    Write-Warning "Bitte in der Exchange Management Shell ausführen!"
    return $false
  }
  return $true
}
...
function Get-ExchangeServerVersions {
  ...
  if (-not $s) { Write-Warning "Keine Exchange-Server gefunden."; return @() }
  Write-Host "`nDetected Exchange Servers:" -ForegroundColor Yellow
  $s | Select Name,Edition,AdminDisplayVersion | Format-Table -AutoSize | Out-Host
  return $s
}

function Normalize-VersionString([string]$text) {
  if ([string]::IsNullOrWhiteSpace($text)) { return $null }
  if ($text -match 'Version\s+(\d+)\.(\d+)') { return ("{0}.{1}" -f [int]$matches[1],[int]$matches[2]) }
  try { $v = [version]$text; return ("{0}.{1}" -f $v.Major,$v.Minor) } catch { $null }
}
...
function Resolve-Servers([array]$servers,[string]$src,[string]$tgt) {
  ...
  $map = $servers | ForEach-Object {
    [pscustomobject]@{
      Name = $_.Name
      Raw  = $_.AdminDisplayVersion.ToString()
      Ver  = Normalize-VersionString ($_.AdminDisplayVersion.ToString())
    }
  }
  ...
  if (-not $source -or -not $target) {
    Write-Host ("Found versions: " + (($map | ForEach-Object { "{0}={1}" -f $_.Name,$_.Ver }) -join ', ')) -ForegroundColor DarkYellow
    return $null
  }
...
function Get-DatabaseInfo([string]$serverName) {
  Get-MailboxDatabase -Server $serverName |
   Select-Object Name,
     @{N='EdbFilePath';E={[string]$_.EdbFilePath}},
     @{N='LogFolderPath';E={[string]$_.LogFolderPath}}
}

function New-DbNameFromOld([string]$oldName) {
  if ($oldName -match '^(.*?)(\d+)$') {
    $p=$matches[1]; $d=$matches[2]; $w=$d.Length; $n=[int]$d+1
    return $p + $n.ToString('D'+$w)
  } else {
    return $oldName + '1'
  }
}
...
function Apply-DriveMap([string]$path) {
  ...
  if (-not $DriveMap) { return $path }
  $drive = ($path -split '(?<=:)',2)[0]
  if ($DriveMap.ContainsKey($drive)) { return $DriveMap[$drive] + $path.Substring(2) }
  return $path
}
...
function Build-TargetPaths([string]$dbName,[string]$oldEdb,[string]$oldLog) {
  if ($PathMode -eq 'MirrorSource') {
    ...
    $dbBase = if ($oldDbFolder) {
      try { Split-Path -Path $oldDbFolder -Parent } catch { $null }
    } else { $null }
    ...
    if ($oldLog -and $oldDbFolder -and $oldLog.StartsWith($oldDbFolder,[System.StringComparison]::OrdinalIgnoreCase)) {
      ...
      $logBase = $null
      if ($oldLog) { try { $logBase = Split-Path -Path (Split-Path -Path $oldLog -Parent) -Parent } catch {} }
...
function Test-RemoteFolder([string]$server,[string]$path) {
  ...
  $p = [string]$path
  try {
    Invoke-Remote -server $server -block {
      param() $pp=$using:p
...
function Prepare-TargetFolders([string]$targetServer,[pscustomobject]$paths) {
  if ($PSCmdlet.ShouldProcess($paths.DbFolder,'Ensure remote DB folder'))   { void }
  if ($PSCmdlet.ShouldProcess($paths.LogFolder,'Ensure remote LOG folder')) { void }
}

function Ensure-UniqueDbName {
    [CmdletBinding()]
    param(
    ...
    try {
        $null = Get-MailboxDatabase -Identity $name -ErrorAction Stop
        return $false  # gefunden -> nicht unique
    }
    catch [Microsoft.Exchange.Configuration.Tasks.ManagementObjectNotFoundException] {
        return $true   # nicht gefunden -> unique
    }
    catch {
        Write-Verbose ("Ensure-UniqueDbName: Unexpected error for '{0}': {1}" -f $name, $_.Exception.Message)
        return $false  # konservativ
    }
}
...
function Create-NewMailboxDatabase([string]$targetServer,[string]$dbName,[pscustomobject]$paths) {
  if (-not (Ensure-UniqueDbName -name $dbName)) {
    Write-Warning "Database '$dbName' already exists. Skipping creation."
    return
  }
  ...
  $params = @{
    Name = $dbName
    Server = $targetServer
    EdbFilePath = $paths.EdbFile
    LogFolderPath = $paths.LogFolder
  }
  if ($ExcludeFromProvisioning) { $params['IsExcludedFromProvisioning'] = $true }

  if ($PSCmdlet.ShouldProcess($dbName,'New-MailboxDatabase')) {
    New-MailboxDatabase @params | Out-Null
    Write-Host "Created DB '$dbName' on '$targetServer' (EDB=$($paths.EdbFile); LOG=$($paths.LogFolder))." -ForegroundColor Green
...
function Expand-SettingsList([string[]]$scope) {
  if (-not $scope -or $scope -contains 'All') { return ($SettingsMap.Values | ForEach-Object { $_ }) }
  $list = @()
  foreach($s in $scope) { if ($SettingsMap.ContainsKey($s)) { $list += $SettingsMap[$s] } }
  $list | Select-Object -Unique
}

function Convert-Quota([object]$q) {
  if ($null -eq $q) { return $null }
  try {
    if ($q.PSObject.TypeNames -contains 'Microsoft.Exchange.Data.Unlimited`1[[Microsoft.Exchange.Data.ByteQuantifiedSize]]' -or
        $q.GetType().FullName -like 'Microsoft.Exchange.Data.Unlimited`1*ByteQuantifiedSize*') {
      if ($q.IsUnlimited) { return 'Unlimited' }
      $s = [string]$q.Value
    } else { $s = [string]$q }
  } catch { $s = [string]$q }

  if ($s -match '^\s*[Uu]nlimited\s*$') { return 'Unlimited' }
  if ($s -match '\((?<bytes>[\d\.,\s]+)\s*bytes\)') {
    $raw = $matches['bytes'] -replace '[^\d]',''
    if ($raw) {
      $bytes = [double]$raw
      if ($bytes -gt 0) { $mb = [math]::Round($bytes/1MB); if ($mb -lt 1) { $mb = 1 }; return ("{0} MB" -f $mb) }
    }
  }
  if ($s -match '^(?<size>\d+[\.,]?\d*)\s*(?<unit>B|KB|MB|GB|TB)$') {
    $num = $matches['size'].Replace(',','.')
    $val = [double]::Parse($num,[Globalization.CultureInfo]::InvariantCulture)
    switch ($matches['unit'].ToUpper()) {
      'TB' { $mb=[math]::Round($val*1024*1024) }
      'GB' { $mb=[math]::Round($val*1024) }
      'MB' { $mb=[math]::Round($val) }
      'KB' { $mb=[math]::Round($val/1024) }
      default { $mb=[math]::Round($val/1MB) }
    }
    if ($mb -lt 1) { $mb=1 }
    return ("{0} MB" -f $mb)
  }
...
function Convert-TimeSpanLike([object]$t) { if ($null -eq $t) { return $null }; return [string]$t }
...
function Get-OABInfo([object]$val) {
  $raw = if ($null -eq $val) { $null } else { [string]$val }
  $trim = Trim-OABDisplay $raw
  $info = [pscustomobject]@{ Raw=$raw; RawTrim=$trim; Name=$null; Dn=$null; Guid=$null; IsNull=([string]::IsNullOrWhiteSpace($raw)); Resolved=$false }
  if ($info.IsNull) { return $info }
  try {
    $oab = $null
    if ($trim -like 'CN=*,CN=Offline Address Lists*') { $oab = Get-OfflineAddressBook -Identity $trim -ErrorAction SilentlyContinue }
    ...
    if ($oab) { $info.Name=$oab.Name; $info.Dn=$oab.DistinguishedName; $info.Guid=$oab.Guid; $info.Resolved=$true }
  } catch {}
  return $info
}
function Normalize-DbOABValue([object]$val) {
  try { if ($val -and $val.DistinguishedName) { return Get-OABInfo $val.DistinguishedName } } catch {}
...
function Get-DbSettings([string]$dbName,[string[]]$properties) {
  ...
  $ht = @{}
  foreach($p in $properties) { try { $ht[$p] = $db.$p } catch { $ht[$p] = $null } }
  [pscustomobject]$ht | Select-Object $properties
}

function Compare-DbSettings([string]$sourceDb,[string]$targetDb,[string[]]$properties) {
  $src = Get-DbSettings -dbName $sourceDb -properties $properties
  $tgt = Get-DbSettings -dbName $targetDb -properties $properties -ErrorAction SilentlyContinue
  if (-not $tgt) { Write-Warning "Target DB '$targetDb' does not exist. Create it first."; return @() }

  $diff = @()
  foreach($p in $properties) {
    if ($p -eq 'OfflineAddressBook') {
      $srcI = Normalize-DbOABValue $src.$p
      $tgtI = Normalize-DbOABValue $tgt.$p
      $same = $false; $reason=''
      if ($srcI.IsNull -and $tgtI.IsNull) { $same=$true; $reason='both null' }
      elseif ( ($srcI.IsNull -and -not $tgtI.IsNull) -or (-not $srcI.IsNull -and $tgtI.IsNull) ) { $same=$false; $reason='XOR null/non-null' }
      else {
        ...
        else { $same = ( ($srcI.RawTrim -ieq $tgtI.RawTrim) -or ( $srcI.Name -and $tgtI.Name -and ($srcI.Name -ieq $tgtI.Name) ) ); $reason='Name/Raw compare' }
      }
      if ($DebugOAB) { Write-Host ("OAB DEBUG [{0}]: SAME={1} REASON={2}" -f $targetDb,$same,$reason) -ForegroundColor DarkYellow }
      if (-not $same) {
        $srcDisp = if ($srcI.Name) { $srcI.Name } else { $srcI.RawTrim }
        $tgtDisp = if ($tgtI.Name) { $tgtI.Name } else { $tgtI.RawTrim }
        $diff += [pscustomobject]@{ Database=$targetDb; Property=$p; SourceValue=$srcDisp; TargetValue=$tgtDisp; SourceIdentity=$srcI.Dn; TargetIdentity=$tgtI.Dn; SourceRaw=$srcI.Raw; TargetRaw=$tgtI.Raw }
      }
    } else {
      $sv=$src.$p; $tv=$tgt.$p
      if ( ([string]$sv) -ne ([string]$tv) ) {
        $diff += [pscustomobject]@{ Database=$targetDb; Property=$p; SourceValue=$sv; TargetValue=$tv }
      }
    }
  }
  return ,$diff
...
function Convert-OABForSet([object]$oab) {
  ...
  if ($i.Dn)   { return $i.Dn }
  elseif ($i.Name) { return $i.Name }
  elseif ($i.RawTrim) { return $i.RawTrim }
  else { return $null }
}

function Convert-ToExchangeValue([string]$prop,$val) {
  switch($prop) {
...
function Apply-DbSettings([string]$targetDb,[object[]]$diffRows) {
  if (-not $diffRows -or $diffRows.Count -eq 0) { return }
  ...
  foreach($d in $diffRows) {
    if ($d.Property -eq 'OfflineAddressBook' -and [string]::IsNullOrWhiteSpace($d.SourceValue)) {
      ...
      $san = Convert-ToExchangeValue -prop $d.Property -val $d.SourceValue
      if ($null -ne $san -and $san -ne '') { $params[$d.Property] = $san }
    }
  }
...
# --- CAS URLs (Compare/Apply) -------------------------------------------------

function Test-IsAbsoluteHttpUrl { param([string]$Url)
...
function Test-ServerBoundUrl { param([string]$Url,[string]$SourceServerName)
...
function Get-CasUrlSnapshot { param([string]$Server)
  $snap=[ordered]@{
    Server=$Server
  ...
  try{ $cas=Get-ClientAccessService -Identity $Server; $snap.AutoDiscoverServiceInternalUri=[string]$cas.AutoDiscoverServiceInternalUri } catch {}
  try{ $x=Get-OwaVirtualDirectory -Server $Server; $snap.OWA_InternalUrl=[string]$x.InternalUrl; $snap.OWA_ExternalUrl=[string]$x.ExternalUrl } catch {}
  try{ $x=Get-EcpVirtualDirectory -Server $Server; $snap.ECP_InternalUrl=[string]$x.InternalUrl; $snap.ECP_ExternalUrl=[string]$x.ExternalUrl } catch {}
  try{ $x=Get-WebServicesVirtualDirectory -Server $Server; $snap.EWS_InternalUrl=[string]$x.InternalUrl; $snap.EWS_ExternalUrl=[string]$x.ExternalUrl } catch {}
  try{ $x=Get-ActiveSyncVirtualDirectory -Server $Server; $snap.EAS_InternalUrl=[string]$x.InternalUrl; $snap.EAS_ExternalUrl=[string]$x.ExternalUrl } catch {}
  try{ $x=Get-OabVirtualDirectory -Server $Server; $snap.OAB_InternalUrl=[string]$x.InternalUrl; $snap.OAB_ExternalUrl=[string]$x.ExternalUrl } catch {}
  try{ $x=Get-MapiVirtualDirectory -Server $Server; $snap.MAPI_InternalUrl=[string]$x.InternalUrl; $snap.MAPI_ExternalUrl=[string]$x.ExternalUrl } catch {}
  try{ $x=Get-OutlookAnywhere -Server $Server; $snap.OutlookAnywhere_InternalHostname=[string]$x.InternalHostname; $snap.OutlookAnywhere_ExternalHostname=[string]$x.ExternalHostname } catch {}
  [pscustomobject]$snap
}
...
function Compare-CasUrls {
  ...
  $rows=@()
  foreach($p in $props) {
    $sv=[string]$Source.$p; $tv=[string]$Target.$p
    $equal=($sv -eq $tv)
    $bothEmpty=([string]::IsNullOrWhiteSpace($sv) -and [string]::IsNullOrWhiteSpace($tv))
    $note=$null
    if ($p -like '*Url' -and (Test-ServerBoundUrl -Url $sv -SourceServerName $SourceServerName)) { $note='ServerBound(Source) – manual adjust recommended' }
...
function Apply-CasUrls {
  param([string]$TargetServer,[psobject]$SourceSnapshot)
  ...
  } else { Write-Host "Skip OWA InternalUrl (invalid/serverbound): '$($SourceSnapshot.OWA_InternalUrl)'" -ForegroundColor DarkYellow }
  ...
  } else { Write-Host "Skip ECP InternalUrl (invalid/serverbound): '$($SourceSnapshot.ECP_InternalUrl)'" -ForegroundColor DarkYellow }
  ...
  } else { Write-Host "Skip EWS InternalUrl (invalid/serverbound): '$($SourceSnapshot.EWS_InternalUrl)'" -ForegroundColor DarkYellow }
  ...
  } else { Write-Host "Skip EAS InternalUrl (invalid/serverbound): '$($SourceSnapshot.EAS_InternalUrl)'" -ForegroundColor DarkYellow }
  ...
  } else { Write-Host "Skip OAB InternalUrl (invalid/serverbound): '$($SourceSnapshot.OAB_InternalUrl)'" -ForegroundColor DarkYellow }
  ...
  } else { Write-Host "Skip MAPI InternalUrl (invalid/serverbound): '$($SourceSnapshot.MAPI_InternalUrl)'" -ForegroundColor DarkYellow }

  # Outlook Anywhere
  $ext=$SourceSnapshot.OutlookAnywhere_ExternalHostname
  $int=$SourceSnapshot.OutlookAnywhere_InternalHostname
  if (-not [string]::IsNullOrWhiteSpace($ext) -or -not [string]::IsNullOrWhiteSpace($int)) {
...
# --- MigrationBatch (Moves) ---------------------------------------------------

function Queue-ArbitrationMigrationBatch {
  param(
    [string]$batchPrefix,
  ...
  # Dry-Run Anzeige
  Write-Host "`nARBITRATION MIGRATION BATCH (Plan):" -ForegroundColor Yellow
  $arbMailboxes | Select-Object Name,RecipientTypeDetails,ServerName,Database,@{N='TargetDb';E={$targetDb}} |
    Format-Table -AutoSize | Out-Host

  if (-not $Approve) {
    Write-Host "DRY-RUN: Batch wird NICHT erstellt. Re-run mit -Approve." -ForegroundColor DarkYellow
    return
  }
  ...
  # Laufzeitprüfung lt. Doku (Parametername in New-MigrationBatch; Archive irrelevant für Arbitration)
  $params = @{
    Name            = $batchName
    Local           = $true
    CSVData         = $csvBytes
  ...
  try {
    ...
    Write-Host ("Created Arbitration MigrationBatch '{0}' -> '{1}'. AutoStart={2}" -f $batchName,$targetDb,$AutoStart) -ForegroundColor Cyan
  } catch {
    Write-Warning "Failed to create Arbitration MigrationBatch '$batchName': $($_.Exception.Message)"
  }

  if (-not $KeepCsv) { Remove-Item $csvPath -Force } else { Write-Host "CSV retained at $csvPath" -ForegroundColor Yellow }
}

function Queue-RegularMigrationBatches {
  param(
    [string]$batchPrefix,
    [string]$NotificationEmails,
    [int]$BadItemLimit,
  ...
  $mailboxes = Get-Mailbox -ResultSize Unlimited |
               Where-Object { $_.RecipientTypeDetails -in @('UserMailbox','SharedMailbox','RoomMailbox','EquipmentMailbox') -and $_.ServerName -ne $pair.Target }
  ...
  foreach($mbx in $mailboxes) {
    ...
    $map += [pscustomobject]@{ Mailbox=$mbx; Type=$mbx.RecipientTypeDetails; TargetDb=$targetDb; SourceDb=$mbx.Database }
  }
  ...
  foreach($grp in $groups) {
    ...
    # Dry-Run
    Write-Host ("`nREGULAR MAILBOX MIGRATION BATCH (Plan): {0}" -f $batchName) -ForegroundColor Yellow
    $grp.Group | ForEach-Object { $_.Mailbox } |
      Select-Object Name,RecipientTypeDetails,ServerName,Database,@{N='TargetDb';E={$targetDb}} |
      Format-Table -AutoSize | Out-Host
    ...
    # Korrekte Archiv-Parameter für MigrationBatch (Plural)
    $hasTargetArchive = (Get-Command New-MigrationBatch).Parameters.ContainsKey('TargetArchiveDatabases')
    ...
    if ($NotificationEmails) { $params['NotificationEmails'] = $NotificationEmails }
    if ($hasTargetArchive)  { $params['TargetArchiveDatabases'] = $targetDb } # Primär + Archiv auf gleiche DB
    ...
    try {
      ...
      Write-Warning "Failed to create MigrationBatch '$batchName': $($_.Exception.Message)"
    }
...
# NEU: Systemmailboxen als MigrationBatch

function Queue-SystemMailboxMigrationBatch {
    ...
    param(
        ...
        [string]$BatchNamePrefix = 'SysMbxMove',
        [switch]$AutoStart,
        [switch]$KeepCsv,
        [switch]$Approve
    )
    ...
    # Ziel-DB prüfen
    try { $null = Get-MailboxDatabase -Identity $TargetDb -ErrorAction Stop }
    catch { Write-Error "Target database '$TargetDb' not found. $_"; return }

    # --- Arbitration-Mailboxen nur vom QUELLserver sammeln ---
    $arb = Get-Mailbox -Arbitration -ResultSize Unlimited |
           Where-Object { $_.ServerName -eq $pair.Source }

    # --- Discovery optional hinzufügen: robust über RecipientTypeDetails + Name-Varianten ---
    $disc = @()
    if ($IncludeDiscovery) {
        $disc = Get-Mailbox -ResultSize Unlimited |
                Where-Object {
                    $_.ServerName -eq $pair.Source -and (
                        $_.RecipientTypeDetails -eq 'DiscoveryMailbox' -or
                        $_.Name -like 'DiscoverySearchMailbox*' -or    # Alias/klassischer Name
                        $_.DisplayName -like 'Discovery Search Mailbox*' # Anzeigename (mit Leerzeichen)
                    )
                }
    }

    # --- zusammenführen und nur aufnehmen, was NICHT schon auf der Ziel-DB liegt ---
    $toInclude = @()
    foreach ($m in ($arb + $disc)) {
        if (-not $m) { continue }
        if ($m.Database -eq $TargetDb) {
            Write-Host ("SKIP: {0} (bereits auf {1})" -f $m.Name,$TargetDb) -ForegroundColor DarkGreen
            continue
        }
        $toInclude += $m
    }

    if ($toInclude.Count -eq 0) {
        Write-Host "Keine Systemmailboxen zu migrieren." -ForegroundColor DarkGreen
        return
    }

    # --- Dry-Run Anzeige ---
    Write-Host "`nSYSTEM-MAILBOX MIGRATION BATCH (Plan):" -ForegroundColor Yellow
    $toInclude | Select-Object Name,RecipientTypeDetails,ServerName,Database,@{N='TargetDb';E={$TargetDb}} |
        Format-Table -AutoSize | Out-Host
    ...
    # --- CSV & Batch erstellen ---
    $batchName = ("{0}_{1:yyyyMMdd-HHmm}" -f $BatchNamePrefix,(Get-Date))
    $csvPath   = Join-Path $env:TEMP ("{0}.csv" -f $batchName)

    $toInclude | Select-Object @{Name='EmailAddress';Expression={ $_.PrimarySmtpAddress }} |
        Export-Csv -Path $csvPath -NoTypeInformation -Encoding UTF8
    ...
    try {
        if ($PSCmdlet.ShouldProcess($batchName,"New-MigrationBatch -> $TargetDb")) {
            New-MigrationBatch @params | Out-Null
            if ($AutoStart) { Start-MigrationBatch -Identity $batchName | Out-Null }
            Write-Host ("Created SystemMailbox MigrationBatch '{0}' -> '{1}'. AutoStart={2}" -f $batchName,$TargetDb,$AutoStart) -ForegroundColor Cyan
        }
    } catch {
        Write-Warning ("Failed to create SystemMailbox MigrationBatch '{0}': {1}" -f $batchName, $_.Exception.Message)
    }
...
if ($IncludeDbs -and $IncludeDbs.Count -gt 0) {
  $sourceDbs = $sourceDbs | Where-Object { $IncludeDbs -contains $_.Name }
}

$plan = foreach($db in $sourceDbs) {
  $newName = New-DbNameFromOld -oldName $db.Name
  $paths   = Build-TargetPaths -dbName $newName -oldEdb $db.EdbFilePath -oldLog $db.LogFolderPath
  $dbOk    = Test-RemoteFolder -server $pair.Target -path $paths.DbFolder
  $logOk   = Test-RemoteFolder -server $pair.Target -path $paths.LogFolder
  [pscustomobject]@{
    SourceDb   = $db.Name
    OldEdb     = $db.EdbFilePath
    OldLog     = $db.LogFolderPath
    TargetDb   = $newName
    TargetEdb  = $paths.EdbFile
    TargetDbDir= $paths.DbFolder
    TargetLog  = $paths.LogFolder
    DbDirExists= [bool]$dbOk
    LogDirExists= [bool]$logOk
  }
}

# ----- PLAN-Gate (Anzeige nur wenn fachlich nötig) -----
$shouldShowPlan = $Interactive -or $PrepareFolders -or $CreateDatabases -or $CompareSettings -or $ApplySettings

if ($shouldShowPlan) {
    Write-Host "`nPLAN (review before execution):" -ForegroundColor Yellow
    $plan | Select SourceDb,OldEdb,OldLog,TargetDb,TargetEdb,TargetDbDir,TargetLog,DbDirExists,LogDirExists |
      Format-Table -AutoSize | Out-Host

    if ($ExportPlan) {
        try {
            $plan | Export-Csv -Path $ExportPlan -NoTypeInformation -Encoding UTF8
            Write-Host "Plan exported to $ExportPlan" -ForegroundColor DarkGreen
        } catch {
...
if ($Interactive) {
  Write-Host "`nInteractive mode: edit target paths (ENTER to keep)." -ForegroundColor Cyan
  $adjusted=@()
  foreach($item in $plan) {
    Write-Host ("`nDB: {0} -> {1}" -f $item.SourceDb,$item.TargetDb) -ForegroundColor Cyan
    $nd = Read-Host ("Target DB folder [{0}]" -f $item.TargetDbDir)
    $nl = Read-Host ("Target LOG folder [{0}]" -f $item.TargetLog)
    $ne = Read-Host ("Target EDB file [{0}]" -f $item.TargetEdb)
    if ($nd) { $item.TargetDbDir = $nd }
    if ($nl) { $item.TargetLog   = $nl }
    if ($ne) { $item.TargetEdb   = $ne }
    $adjusted += $item
  }
  $plan = $adjusted
  Write-Host "`nAdjusted PLAN:" -ForegroundColor Yellow
  $plan | Select SourceDb,TargetDb,TargetEdb,TargetDbDir,TargetLog | Format-Table -AutoSize | Out-Host
}

# NEU: Systemmailboxen per MigrationBatch
if ($QueueSystemMailboxBatch -and $SystemMailboxTargetDb) {
  Queue-SystemMailboxMigrationBatch -TargetDb $SystemMailboxTargetDb -IncludeDiscovery:$IncludeDiscoverySystemMailbox -BatchNamePrefix "SysMbx" -AutoStart:$AutoStart -KeepCsv:$KeepCsv -Approve:$Approve
}

# Dry-Run: ohne -Approve keine schreibenden Aktionen (außer Compare/Export)
if (-not $Approve -and -not $CompareSettings -and -not $CompareCasUrls) {
    if ($QueueMoves) {
        if ($Arbitration) {
        ...
        foreach($arb in $arbMailboxes) {
            ...
            if (-not $targetDb -and $plan.Count -gt 0) { $targetDb = $plan[0].TargetDb }
            ...
            foreach($item in $plan) {
                ...
                # Wenn ForceTargetDb angegeben ist, Dry-Run-Ziel entsprechend überschreiben
                $dryTarget = if ($ForceTargetDb) { $ForceTargetDb } else { $item.TargetDb }
                foreach($mbx in $mailboxes) {
                    Write-Host ("{0} ({1}) -> {2}" -f $mbx.DisplayName,$mbx.RecipientTypeDetails,$dryTarget) -ForegroundColor Yellow
                }
            }
        }
    }
  Write-Host "`nNo changes executed. Re-run with -Approve and/or -CompareSettings / -CompareCasUrls when ready." -ForegroundColor DarkYellow
  return
}

# Schreibende Schritte
foreach($item in $plan) {
  $paths = [pscustomobject]@{ DbFolder=$item.TargetDbDir; LogFolder=$item.TargetLog; EdbFile=$item.TargetEdb }
  if ($Approve -and ($PrepareFolders -or $CreateDatabases)) {
    Prepare-TargetFolders -targetServer $pair.Target -paths $paths
    if ($CreateDatabases) { Create-NewMailboxDatabase -targetServer $pair.Target -dbName $item.TargetDb -paths $paths }
  }
}

# Settings Compare/Apply
if ($CompareSettings -or $ApplySettings) {
  $props = Expand-SettingsList -scope $SettingsScope
  $allDiffs=@()
  Write-Host "`nDB SETTINGS DIFFS:" -ForegroundColor Yellow

  foreach($item in $plan) {
    $exists = -not (Ensure-UniqueDbName -name $item.TargetDb)
    if (-not $exists) { Write-Host "Target DB '$($item.TargetDb)' not found (create first)." -ForegroundColor DarkYellow; continue }
    $diff  = Compare-DbSettings -sourceDb $item.SourceDb -targetDb $item.TargetDb -properties $props
    $diffList = @($diff)
    if ($diffList.Count -gt 0) {
      $diffList | Format-Table * -AutoSize | Out-Host
      $allDiffs += $diffList
      if ($Approve -and $ApplySettings) { Apply-DbSettings -targetDb $item.TargetDb -diffRows $diffList }
    } else {
  ...
  if ($ExportDiffs -and @($allDiffs).Count -gt 0) {
    ...
    catch { Write-Warning "Export diffs failed: $($_.Exception.Message)" }
  }
}
...
# Benutzer/Arbitration Moves
if ($Approve -and $QueueMoves) {
  if ($Arbitration) {
    $targetDb = ($plan | Where-Object { $_.SourceDb -like '*Verwaltung*' }).TargetDb
    if (-not $targetDb) { $targetDb = $FallbackTargetDb }
    Queue-ArbitrationMigrationBatch -batchPrefix $BatchNamePrefix -NotificationEmails $NotifyEmail -BadItemLimit $BadItemLimit -TargetDb $targetDb -SplitArbitrationBySourceDb:$SplitArbitrationBySourceDb -AutoStart:$AutoStart -KeepCsv:$KeepCsv -ForceTargetDb $ForceTargetDb -Approve:$Approve
  } else {
    Queue-RegularMigrationBatches -batchPrefix $BatchNamePrefix -NotificationEmails $NotifyEmail -BadItemLimit $BadItemLimit -AutoStart:$AutoStart -KeepCsv:$KeepCsv -FallbackTargetDb $FallbackTargetDb -ForceTargetDb $ForceTargetDb -Approve:$Approve
  }
}
